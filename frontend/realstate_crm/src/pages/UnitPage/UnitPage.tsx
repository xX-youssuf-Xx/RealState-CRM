...
import { type Unit, type UnitStatus, unitStatusTranslations, type FilterOptions } from "../../types/units"
...
const ProjectUnitsPage: React.FC = () => {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    if (name === "status" && value === "SOLD" && !formData.sold_date) {
      ...
      setFormData((prev) => ({ ...prev, [name]: value as UnitStatus, sold_date: today }))
    } else if (name === "status" && value !== "SOLD") {
      // If status is changed from SOLD, clear sold_date
      setFormData((prev) => ({ ...prev, [name]: value as UnitStatus, sold_date: null }))
    } else {
  ...
  const handleFilterChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    if (name === "status") {
      setFilterOptions((prev) => ({ ...prev, [name]: value ? (value as UnitStatus) : null }))
    } else {
      // For numeric inputs
    ...
    <div className={styles.unitsPage}>
      ...
      {isInfoModalOpen && selectedUnit && (
        <div className={styles.modalOverlay}>
          <div className={styles.modal}>
            <div
              className={`${styles.modalHeader} ${selectedUnit.status ? styles[selectedUnit.status.toLowerCase() + "Header"] : ""}`}
            >
            ...
            <div className={styles.infoContent}>
              ...
              <div className={styles.infoSection}>
                ...
                <div className={styles.infoItem}>
                  ...
                  <span
                    className={`${styles.infoValue} ${selectedUnit.status ? styles[selectedUnit.status.toLowerCase() + "Status"] : ""}`}
                  >
