...
export const useGetActionsByUnitId = () => {
  const { isLoading, error, data, setError, _execute } = useApiFetch<Action[]>();

  const execute = useCallback(async (unitId: number, options: Record<string, any> = {}) => {
    try {
      const data = await _execute(`actions/unit/${unitId}`, "GET", null, options);
      return data;
    } catch (e) {
      setError(e);
      throw e;
